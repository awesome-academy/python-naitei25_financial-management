...
@register.simple_tag
def render_pagination_simple(page_obj, query_param="page", anchor=""):
    if not page_obj:
        return ""
    ...
    if page_obj.has_previous():
        prev_url = f"?{query_param}={page_obj.previous_page_number()}{anchor}"
        parts.append(
            format_html(
    ...
    if page_obj.has_next():
        next_url = f"?{query_param}={page_obj.next_page_number()}{anchor}"
        parts.append(
            format_html(
                '<a href="{}" class="px-3 py-1 mx-1 bg-gray-200 rounded hover:bg-gray-300">{}</a>',
                next_url,
